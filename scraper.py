    ...
    async def _run_targeted_playwright(self, url: str, psi_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        async with async_playwright() as p:
            ...
            try:
                ...
                results["click_to_call_present"] = has_call_element
                ...
                ai_flags = await page.evaluate("""() => {
                    ...
                    flags.shadcn_markers = document.querySelectorAll('[class*="shadcn"], [data-slot], [class*="cn("]').length > 0;
                    ...
                    flags.generic_headline = headings.some(h => genericPatterns.some(pat => pat.test(h.innerText)));
                    ...
                    flags.has_custom_photos = Array.from(document.querySelectorAll("img")).map(i => i.src || "").some(src => !stockDomains.some(d => src.includes(d)));
                ...
                ai_score = 0.0
                if ai_flags.get("tailwind_classes", 0) > 20: ai_score += 25.0
                if ai_flags.get("shadcn_markers"): ai_score += 20.0
                if ai_flags.get("lucide_icons", 0) > 5: ai_score += 15.0
                if ai_flags.get("generic_headline"): ai_score += 15.0
                if ai_flags.get("unlinked_forms", 0) > 0: ai_score += 10.0
                if ai_flags.get("has_custom_photos"): ai_score -= 15.0
                if results.get("has_meta_pixel"): ai_score -= 10.0

                ai_score += self._analyze_text_ai_patterns(visible_text)
                results["ai_spectrum_pct"] = max(0.0, min(100.0, round(ai_score, 1)))

                cms = ""
                if "wp-content" in content_lower: cms = "WordPress"
                elif "shopify" in content_lower or "myshopify" in content_lower: cms = "Shopify"
                elif "wix" in content_lower: cms = "Wix"
                elif "squarespace" in content_lower: cms = "Squarespace"
                elif "webflow" in content_lower: cms = "Webflow"
                elif ai_flags.get("tailwind_classes", 0) > 10: cms = "Modern Stack"
