...
class ReportGenerator:
    ...
    - Archives snapshot to Vault (configurable for persistent cloud storage).
    ...
    def generate_admin_master_report(self, audit_data: Dict[str, Any], scan_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        sorted_leaks = sorted(
            ...
            key=lambda x: (x.get("severity_score", 0), x.get("severity_factor", 0.0)),
            reverse=True
        )

        top_leaks = sorted_leaks[:6]
        ...
        for leak in top_leaks:
            ...
            severity_factor = leak.get("severity_factor", 1.0)
            
            enriched_leaks.append({
                "id": leak.get("id"),
                "severity_score": leak.get("severity_score", 0),
        ...
        biz_type = audit_data.get("business_type", "general").upper()
        rev_leak = audit_data.get("revenue_leak", {}).get("est_annual_revenue_leak", "N/A")

        admin_payload = {
            "report_type": "ADMIN_LEAD_ALERT",
            "generated_at": datetime.datetime.now(datetime.timezone.utc).isoformat(),
            "target_domain": audit_data.get("target_domain", "Unknown"),
    ...
    def _build_3_angle_solutions(self, leak_name: str, category: str, scan_data: Dict[str, Any]) -> Dict[str, str]:
        ...
        lname_lower = leak_name.lower()

        if any(term in lname_lower for term in ["latency", "core web vitals", "performance", "speed"]):
            return {
        ...
        elif any(term in lname_lower for term in ["proof", "social proof", "trust", "review", "testimonial"]):
            return {
        ...
        elif any(term in lname_lower for term in ["organic", "content", "h1", "heading", "seo"]):
            return {
        ...
        elif any(term in lname_lower for term in ["ssl", "https"]):
            return {
        ...
        elif any(term in lname_lower for term in ["call", "tap target", "phone"]):
            return {
        ...
        elif any(term in lname_lower for term in ["cta", "cart", "form", "support"]):
            return {
    ...
    def _build_50_checkpoints(self, scan_data: Dict[str, Any], audit_data: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        perf_score = scan.get("performance_score", 0)
        seo_score = scan.get("google_seo_score", 0)
        meta_desc = scan.get("meta_description", "")
        h1_tags = scan.get("h1_tags", [])
        title_str = scan.get("title", "")
        ...
        checkpoints.append({"id": 34, "check": "Images Have Alt Text", "status": "PASS" if scan.get("missing_alt_images", 0) == 0 else "FAIL", "category": "seo_technical"})
        checkpoints.append({"id": 35, "check": "Lazy Loading on Images", "status": "PASS", "category": "seo_technical"})

        # Category 3: Content & E-E-A-T (36-50)
        ai_pct = scan.get("ai_spectrum_pct", 0)
        page_len = scan.get("page_content_len", 0)
        ...
        checkpoints.append({"id": 43, "check": "No Unlinked Forms", "status": "PASS" if ai_flags.get("unlinked_forms", 0) == 0 else "FAIL", "category": "content_eeat"})
    ...
    def _build_email_html(self, report: Dict[str, Any]) -> str:
        ...
        methodology = report.get("scoring_methodology", {})
        score_impact = report.get("score_level_impact", {})
        ...
        leaks_html = ""
        top_leaks = report.get("top_6_financial_leaks", [])
        for i, leak in enumerate(top_leaks, 1):
            if not isinstance(leak, dict):
                continue
            angles = leak.get("solutions_3_angles", {})
            sev_label = leak.get("severity_label", "MODERATE")
            sev_factor = leak.get("severity_factor", 1.0)
...
<div style="max-width:640px; margin:0 auto; background:#fff; padding:32px 28px;">
    ...
    def archive_to_vault(self, target_domain: str, admin_report: Dict[str, Any], raw_scan_data: Dict[str, Any]) -> str:
        ...
        filename = f"{self.vault_dir}/{sanitized_domain}_{timestamp}.json"
